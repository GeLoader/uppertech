<?php 
// Email settings 
$toEmail = 'geloader0@gmail.com'; // Recipient email 
$from = $email; // Sender email 
$fromName = 'UpperTech'; // Sender name 
...
if(isset($_POST['submit'])){ 
    ...
    $postData = $_POST; 
    $name = trim($_POST['Contact-5-Name-2']); 
    $email = trim($_POST['Contact-5-Email-2']); 
    $subject = 'Contact Form Submission';
    $message = trim($_POST['Contact-5-Message-2']); 
    ...
    if(empty($valErr)){ 
        ...
        if(!empty($_FILES["Contact-5-File"]["Contact-5-Name-2"])){ 
             
            // File path config 
            $targetDir = $attachmentUploadDir; 
            $fileName = basename($_FILES["Contact-5-File"]["Contact-5-Name-2"]); 
            ...
            if(in_array($fileType, $allowFileTypes)){ 
                ...
                if(move_uploaded_file($_FILES["Contact-5-File"]["tmp_name"], $targetFilePath)){ 
...
//     if ($success) {
//         header('Location: contact-1.html');
//         ob_end_flush();
//     } else {
